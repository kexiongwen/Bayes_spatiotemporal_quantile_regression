...
# 随机选择一个站点
# random_idx = np.random.randint(0, ds.dims[station_dim])
# print(f"随机选中的站点索引：{random_idx}")
random_idx = 500
print(f"指定的站点索引：{random_idx}")
# 提取该站点完整时间序列
site_data_raw = ds[wind_var].isel({station_dim: random_idx})
site_data = site_data_raw.resample(time='1D').mean()
print("已将数据重采样为每日平均值。")
# 筛选 2021 年数据
data_2021_03_06 = site_data.sel(time=slice('2021-03-01', '2021-06-30'))


print(f"2021年3月-6月数据点数：{len(data_2021_03_06.time)}")


y = data_2021_03_06.values
...
plt.plot(np.arange(N, N+H), future_true, 's-', label="True Future (7)", color='blue', markersize=6, linewidth=2)
